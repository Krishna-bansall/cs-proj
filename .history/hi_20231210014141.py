...
def main():
    ...
    if page_option == "Edit Document":
        ...
        # Option to change the header
        new_header = st.text_input("Enter new header for the document:", load_text_from_local_storage())
        st.session_state.new_header = new_header

        if st.button("Change Header"):
            st.title(st.session_state.new_header)
