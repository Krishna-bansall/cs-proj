...
def save_text_to_local_storage(text, filename="local_document.txt"):
    spinner_message = "Saving... Please wait."
...
def main():
    ...
    if page_option == "Edit Document":
        ...
        clear_todo_list_button = st.button("Clear Todo List")
        ...
        for i, todo_item in enumerate(st.session_state.get("todo_list", []), start=1):
            ...
            # Add checkbox to mark task as complete (strikethrough)
            checkbox_label = st.checkbox(label=task, value=complete, key=f"checkbox_{i}")
            todo_item["complete"] = checkbox_label

            # Add button to remove task
            remove_button_label = f"Remove Task {i}"
            remove_button = st.button(remove_button_label, key=f"remove_button_{i}")
            if remove_button:
                st.session_state.todo_list.remove(todo_item)

        # Button to clear the entire todo list
