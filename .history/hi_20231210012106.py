...
def load_text_from_local_storage():
    try:
        with st.empty():
            spinner = st.spinner("Loading... Please wait.")
...
def create_backup():
    ...
    with st.empty():
        spinner = st.spinner("Creating Backup... Please wait.")
        time.sleep(random.uniform(0.01, 0.1))  # Random time between 0.01 and 0.1 seconds
        spinner.success(f"Backup created successfully at {backup_path}")
