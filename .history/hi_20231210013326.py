Sure, I can help with that. Here's your updated code:
...
In this updated code, I've removed the theme selection part because Streamlit doesn't support dynamic theme changes through the `set_page_config()` function. If you want to set a theme for your Streamlit app, you need to define it in the `[theme]` section of a `.streamlit/config.toml` file. Please note that the `set_page_config()` function must be the first Streamlit command used on an app page, and must only be set once per page. If you're still having issues, please let me know! I'm here to help. 😊